          ...
          <div class="form-group">
            ...
            <?php
              if(sizeof($scanData['data']) > 0) {                       // Scan Data, present to user as nice as we can :)
                ...
                $displayed=false;
                $dt = new DateTime();
                echo "[" . $dt->format('Y-m-d H:i:s') . "] 启动SQLMAP API扫描\n";
                    ...
                    switch($dataEntry['type']) {
                      case "0":
                        ...
                        if(($dataEntry['status'] == 1) && (sizeof($dataEntry['value']) > 0)) {
                          if(!$displayed) {
                            echo "[*] 目标网址: " . htmlentities($options_to_enable['url'], ENT_QUOTES, 'UTF-8') . "\n";
                      ...
                      case "2":
                        ...
                        if($dataEntry['status'] == 1) {
                          echo "[*] 数据库Banner: " . htmlentities($dataEntry['value'], ENT_QUOTES, 'UTF-8') . "\n";
                      ...
                      case "3":
                        // Current DB User
                        if($dataEntry['status'] == 1) {
                          echo "[*] 当前数据库用户: " . htmlentities($dataEntry['value'], ENT_QUOTES, 'UTF-8') . "\n";
                      ...
                      case "4":
                        // Current DB
                        if($dataEntry['status'] == 1) {
                          echo "[*] 当前数据库: " . htmlentities($dataEntry['value'], ENT_QUOTES, 'UTF-8') . "\n";
                      ...
                      case "5":
                        // Hostname
                        if($dataEntry['status'] == 1) {
                          echo "[*] 主机名: " . htmlentities($dataEntry['value'], ENT_QUOTES, 'UTF-8') . "\n";
